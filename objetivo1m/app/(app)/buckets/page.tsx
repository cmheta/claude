...
export default function BucketsPage() {
  ...
  const load = useCallback(async () => {
    ...
    // Auto-create default buckets if missing
    if (!bData || bData.length === 0) {
      const defaults = [
        { user_id: user.id, year: currentYear, bucket_name: "vacaciones", annual_budget_usd: 5040, monthly_contribution_usd: 420 },
        { user_id: user.id, year: currentYear, bucket_name: "educacion", annual_budget_usd: 1800, monthly_contribution_usd: 150 },
        { user_id: user.id, year: currentYear, bucket_name: "fun", annual_budget_usd: 2664, monthly_contribution_usd: 222 },
        { user_id: user.id, year: currentYear, bucket_name: "navidad", annual_budget_usd: 576, monthly_contribution_usd: 48 },
      ]
      await supabase.from("buckets").insert(defaults)
      const { data: fresh } = await supabase.from("buckets").select("*").eq("user_id", user.id).eq("year", currentYear)
      buildBuckets(fresh ?? [], tData ?? [])
    } else {
      buildBuckets(bData, tData ?? [])
    }
    setLoading(false)
  }, [currentYear, supabase])

  function buildBuckets(bData: Bucket[], tData: BucketTransaction[]) {
    const withBalance: BucketWithBalance[] = bData.map((b) => {
      const accumulated = b.monthly_contribution_usd * currentMonth
      const txs = tData.filter((t) => t.bucket_id === b.id)
      const spent = txs.filter((t) => t.amount_usd < 0).reduce((s, t) => s + Math.abs(t.amount_usd), 0)
      const extraContribs = txs.filter((t) => t.amount_usd > 0).reduce((s, t) => s + t.amount_usd, 0)
      const balance = accumulated + extraContribs - spent
      return { ...b, accumulated, spent, balance }
    })
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Pots {currentYear}</h1>
          <p className="text-sm text-slate-500 mt-0.5">
            Budget anual: {formatUSD(totalBudget)} · Gastado: {formatUSD(totalSpent)}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {buckets.map((b) => {
          const spentPct = Math.min((b.spent / b.accumulated) * 100, 100) || 0
              ...
              <CardContent className="space-y-3">
                ...
                <div className="space-y-1">
                  <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div
                      className={`h-full ${isOver ? "bg-red-500" : color} rounded-full transition-all`}
                      style={{ width: `${spentPct}%` }}
                    />
                  </div>
                  <p className="text-xs text-slate-400">
                    {spentPct.toFixed(0)}% del acumulado · ${b.monthly_contribution_usd}/mo
                  </p>
                </div>
                ...
                <Button
                  ...
                  size="sm"
                  className="w-full"
                  onClick={() => { setAddingTo(b.id); setTxAmount(""); setTxDesc("") }}
                >
                  + Añadir gasto
                </Button>
      ...
      {addingTo && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <Card className="w-full max-w-sm">
            <CardHeader>
              <CardTitle className="text-sm">Añadir gasto</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder="Ej: Vuelo a Barcelona"
                />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="flex gap-2 pt-1">
                <Button variant="outline" className="flex-1" onClick={() => setAddingTo(null)}>
                  Cancelar
                </Button>
                <Button className="flex-1" onClick={addTransaction} disabled={saving}>
